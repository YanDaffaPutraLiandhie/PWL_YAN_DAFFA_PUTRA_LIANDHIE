...
public function store(Request $request)
    {
        ...
        $validator = \Validator::make($request->all(), [
            'supplier_kode' => 'required|string|max:10|unique:suppliers,supplier_kode',
            'supplier_nama' => 'required|string|max:200',
...
public function update_ajax(Request $request, string $id)
    ...
    {
        // Ambil kode terakhir dari supplier
        $lastSupplier = SupplierModel::orderBy('supplier_kode', 'desc')->first();
        
        if (!$lastSupplier) {
            $kodeBaru = 'SUP001';  // Jika belum ada data supplier, mulai dari SUP001
        } else {
            // Ambil angka terakhir setelah 'SUP'
            $lastKode = (int) substr($lastSupplier->supplier_kode, 3);
            
            // Tambahkan 1 ke angka terakhir dan format menjadi 3 digit
            $kodeBaru = 'SUP' . str_pad($lastKode + 1, 3, '0', STR_PAD_LEFT);
        }
        
        // Kembalikan kode baru sebagai response JSON
        return response()->json(['kode' => $kodeBaru]);
    }
}